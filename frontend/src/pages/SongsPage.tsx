...
export default function SongsPage() {
  ...
  const handleAddSong = async (songId: number) => {
    const { error } = await tryCatch(sessionService.addSong(songId));

    if (error) {
      console.error("Failed to add song:", error);
      toast.error(
        "Failed to add song. Make sure you are in an active session."
      );
      return;
    }
        ...
        {songsData && songsData.totalPages > 1 && (
          <div className="flex justify-center items-center space-x-2">
            <button
              ...
              disabled={page === 1}
              className="hover:bg-gray-50 disabled:opacity-50 px-4 py-2 border border-gray-300 rounded-lg disabled:cursor-not-allowed"
            >
              Previous
            </button>
